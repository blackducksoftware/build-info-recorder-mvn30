...
public class DependencyRecorder_3_0_X extends AbstractEventSpy {
	...
	public void init(final Context context) throws Exception {
		...
		final Properties contextUserProperties = (Properties) data.get(Recorder_3_0_Loader.CONTEXT_USER_PROPERTIES);
		final Properties contextSystemProperties = (Properties) data.get(Recorder_3_0_Loader.CONTEXT_SYSTEM_PROPERTIES);

		buildId = contextUserProperties.getProperty(Recorder_3_0_Loader.PROPERTY_BUILD_ID);
		if (StringUtils.isEmpty(buildId)) {
			buildId = contextSystemProperties.getProperty(Recorder_3_0_Loader.PROPERTY_BUILD_ID);
		}
		...
		workingDirectory = (String) data.get(Recorder_3_0_Loader.PROPERTY_WORKING_DIRECTORY);
	...
	private void handleBdioOutput() throws IOException {
		...
		file = new File(file, Recorder_3_0_Loader.TARGET);
		file = new File(file, projectGav.getArtifactId() + Recorder_3_0_Loader.BDIO_FILE_SUFFIX);

		final List<com.blackducksoftware.integration.build.bdio.DependencyNode> children = new ArrayList<>();
		final com.blackducksoftware.integration.build.bdio.DependencyNode root = new com.blackducksoftware.integration.build.bdio.DependencyNode(
				projectGav, children);

		for (final DependencyNode child : rootMavenDependencyNode.getChildren()) {
	...
	private com.blackducksoftware.integration.build.bdio.DependencyNode createCommonDependencyNode(
		...
		final String groupId = mavenDependencyNode.getDependency().getArtifact().getGroupId();
		final String artifactId = mavenDependencyNode.getDependency().getArtifact().getArtifactId();
		final String version = mavenDependencyNode.getDependency().getArtifact().getVersion();
